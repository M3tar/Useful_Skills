...
- 2026-02-04：march-text-polish 增加“未选类型先询问再产出”的强制规则，并同步到 README。
- 2026-02-05：march-text-polish 明确“只有给编号才算已选类型”，仅说“优化/扩写/翻译/整理”等仍需先询问。
...
### 核心规则（简版）
- 未明确选类型时：只展示类型图 + 推荐与理由 + 询问类型，不产出任何备选。
- 明确选类型仅限用户给出编号（1-5）或明确说“选择/类型/按/用 X”；仅说“优化/扩写/翻译/整理”等也视为未选。
